...
// ── Status badge config for request status ────────────────────
const STATUS_CONFIG = {
...
const MyActivity = () => {
  ...
  const [closingId,    setClosingId]    = useState(null); // tracks which request is being closed
  const [activeTab,    setActiveTab]    = useState('requests'); // 'requests' or 'donor'
  ...
  const fetchMyData = async () => {
    setLoading(true);
    try {
      // Fetch both my requests and donor profile at the same time
                ...
                return (
                  <div key={request._id}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {/* Close request — only for open requests */}
                      {request.status === 'open' && (
                        <button
              ...
              <div className="border-t pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(donorProfile.healthFlags || {}).map(([flag, value]) => {
                    // Only show flags that are true (active conditions)
                  ...
                  {/* Show "All clear" if no health flags are active */}
