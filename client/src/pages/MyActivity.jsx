          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                // ── Find accepted donor to show feedback modal ──
                const fulfilledReq = myRequests.find(
                  r => r._id === requestId
                );
                const acceptedRes = fulfilledReq?.respondents?.find(
                  r => r.status === 'accepted'
                );

                await fetchMyData();

                // Show feedback modal if donor accepted
                if (acceptedRes?.donorId) {
                  setFeedbackModal({
                    isOpen:    true,
                    requestId: requestId,
                    donorId:   acceptedRes.donorId,
                    hospital:  fulfilledReq?.hospital
                  });
                }
            ...
            className="flex-1 py-1.5 rounded-lg text-white
    ...
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Your blood requests and donor profile
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-4 text-xs
                      ...
                      <span>
                        👥 {request.respondents?.length || 0} donor(s)
                      </span>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className="px-4 py-1.5 text-sm rounded-lg border-2
                      ...
                      {/* Mark Fulfilled — only for open requests */}
                      {request.status === 'open' && (
                        <button
                      ...
                      {/* Rate Donor — for fulfilled requests */}
                      {request.status === 'fulfilled' &&
                       request.respondents?.some(
                         r => r.status === 'accepted'
                       ) && (
                        <button
                          onClick={() => {
                            const accepted = request.respondents.find(
                              r => r.status === 'accepted'
                            );
                            setFeedbackModal({
                              isOpen:    true,
                              requestId: request._id,
                              donorId:   accepted?.donorId,
        ...
        <div className="bg-white rounded-2xl shadow p-8">
          {donorProfile ? (
            <div>
              ...
              {/* ── Reputation Score ── */}
              {(donorProfile.positiveRatings + donorProfile.negativeRatings)
                > 0 && (
                <div
                ...
                >
                  ...
                  <p
                    ...
                    style={{
                      color: donorProfile.reputationScore >= 70
                        ? '#15803D' : '#C0171D'
                    }}
                  >
                    {donorProfile.reputationScore}%
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Based on {donorProfile.positiveRatings +
                              donorProfile.negativeRatings} rating(s)
                    — 👍 {donorProfile.positiveRatings}
                    &nbsp;👎 {donorProfile.negativeRatings}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                {[
                  {
                    label: 'Blood Type',
                    value: donorProfile.bloodType,
                    icon:  '🩸'
                  },
                  {
                    label: 'District',
                    value: donorProfile.location?.district,
                    icon:  '📍'
                  },
                  {
                    label: 'City',
                    value: donorProfile.location?.city || '—',
                    icon:  '🏙️'
                  },
                  {
                    label: 'Age',
              ...
              <div className="border-t pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(donorProfile.healthFlags || {})
                    .map(([flag, value]) => {
                      if (!value) return null;
                      const labels = {
                        hasFever:          '🤒 Fever',
                        onAntibiotics:     '💊 Antibiotics',
                        recentSurgery:     '🏥 Recent Surgery',
                        recentTattoo:      '🖊️ Recent Tattoo',
                        isPregnant:        '🤰 Pregnant',
                        hasChronicDisease: '❤️‍🩹 Chronic Disease',
                      };
                      return (
                        <span
                          key={flag}
                  ...
                  {!Object.values(donorProfile.healthFlags || {})
                    .some(Boolean) && (
      ...
      <FeedbackModal
        ...
        donorId={feedbackModal.donorId}
        hospital={feedbackModal.hospital}
        onFeedbackSubmitted={fetchMyData}
      />
    </div>
  );
};
